...
func (e *exprTypeFinder) DataTypeForExpr(expr logicalplan.Expr) (arrow.DataType, error) {
	// Use the compatibility layer to ensure we're working with v18 types
	dt, err := logicalplan.DataTypeForExprWithSchema(expr, e.s)
	if err != nil {
		return nil, err
	}
	return convert.ConvertArrowType(dt), nil
}
